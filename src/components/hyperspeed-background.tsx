  ...
  init() {
    ...
    material.onBeforeCompile = shader => {
      shader.vertexShader = shader.vertexShader.replace(
        ...
        typeof this.options.distortion === 'object' ? this.options.distortion.getDistortion : distortion_vertex
      );
    };
...
const roadMarkings_fragment = `
    ...
    // How much empty space between lines on the Y axis
    float brokenLinesLength = 1. / uBrokenLinesLengthPercentage;
    
    float brokenLinesY = step(1., mod(uv.y * 10., brokenLinesLength));
    
    float brokenLinesX = 0.;
    for(float i = 1.; i < uLanes; i++){
      float laneX = laneWidth * i;
      brokenLinesX += step(laneX - brokenLineWidth/2., uv.x) - step(laneX + brokenLineWidth/2., uv.x);
    }
    
    color = mix(color, uBrokenLinesColor, brokenLinesX * brokenLinesY);
`;
